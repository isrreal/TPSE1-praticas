...
void ISR_Handler() { 
    ...
    if (irq_number == 95) // 95 é o número de interrupção DTIMER para o módulo GPIO1
	timerIrqHandler();
...
// TIMER interruption functions implementation 
...
void timerSetup(bool use_interrupt) {
     //  Clock enable for DMTIMER7 
    HWREG(CM_PER_TIMER7_CLKCTRL) |= 0x2;

    // Check clock enable for DMTIMER7  
...
void delay(volatile unsigned int delay_value) {
	if (use_interrupt) {
		unsigned int countVal = TIMER_OVERFLOW - (delay_value * TIMER_1MS_COUNT);
